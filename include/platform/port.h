...
/**
 * Reset host m4a runtime-audio diagnostics counters (frames generated,
 * non-zero frames, mix ticks). Call at startup from a quiescent point
 * so smoke-test self-check traffic does not pollute runtime assertions.
 */
void Port_M4AAudioDiagReset(void);

/** Total stereo frames generated by the host m4a mixer bridge. */
uint64_t Port_M4AAudioFramesGenerated(void);

/** Number of generated stereo frames whose mixed sample was non-zero. */
uint64_t Port_M4AAudioFramesNonZero(void);

/** Total number of host SoundMainBTM mixer ticks executed. */
uint64_t Port_M4AAudioMixTicks(void);

/** Number of m4a song-start requests observed on the host path. */
