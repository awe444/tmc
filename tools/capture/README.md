...
| `TMC_SCRIPT_TRACE=1` | the cutscene orchestrator's instruction walk — entity, room, pointer, raw word, decoded command index and size, one line per command — plus a per-entity `[prio]` line carrying `EntityDisabled`, `ENT_SCRIPTED`, action, the priority handler's state and an `ObjectUpdate` **call count**. Tag by entity: several orchestrators run in one recording and a trace that does not distinguish them will conflate their walks, which is how B43's first diagnosis went wrong. It answers *did the dispatcher decline to run it*; `TMC_ENT_WATCH` answers *was it reached at all* |
| `TMC_ENT_TRACE=1` | the cutscene orchestrator's deletions with the caller, every `SetRoomFlag`, and a per-frame scan of `gEntityScriptCtxTable` for two entities sharing one script context. Built for B43/#93: it is what showed the orchestrator *self*-deletes, that `SetRoomFlag` never runs, and that no context is shared |
| `TMC_ENT_WATCH=1` | per frame and per cutscene orchestrator: **which `gEntityLists` index holds it** and **whether that frame's iteration reached it**, printed on change, plus the frame and caller of every `sub_0805E958`/`sub_0805E974` list-head swap and a report of any chain that ends in NULL instead of its sentinel. Three things stop an entity being updated and the entity distinguishes only one: unlinked (`prev`/`next` change), written out of the head (nothing about it changes), dispatcher declined (nothing about the list changes). `inList=-1` with the links intact is the middle one — B43 |
| `TMC_SINK_TRACE=1` | the player and `OBJECT_70` per frame while the player is put *behind* the ground layer (swamp, stairs): world/screen position, sprite and frame, `spriteSettings.draw`, the raw `spriteOrientation` byte and the **OAM priority** it becomes, `spriteVramOffset`, and the OAM entries each entity actually emitted. Plus every gfx-group load that lands on OBJ VRAM tile 133, in order, with the frame and whether its *source* bytes there are blank — `AssetLogOnce` dedupes the ordinary `[ASSET]` line, so it cannot show which load was last. An entity that emits entries and still shows nothing is drawing blank tiles, which is a different fault from one that emits none — B45. Dumps sprite 167's frames 0..15 once, because "twelve 8x8 pieces all on one tile" needs its neighbours beside it to be read as a mask rather than as data read off the end of a table |
| `TMC_SINK_FILL133=1` | paints OBJ VRAM tile 133 solid, so the mask `OBJECT_70` draws there has visible geometry. Pair it with `TMC_DISABLE_BG2=1`, which is what makes the player visible at all while he is at OAM priority 3. Answers *which part of him would this cover* — the question a blank tile hides, and the answer retired B45's first two theories: flat coverage across the sink, and the wrong end of him. Diagnostic only |
| `TMC_SINK_TRACE=1` also dumps the finished OAM | on the 1st, 21st and 61st frame of a sink, in the same decode mGBA's debugger prints, so the port and the real game diff entry for entry. See `tools/mgba/README.md` |
| `TMC_FADE_TRACE=1` | every `SetFade` / `SetFadeInverted` with type, speed and calling address (resolve with `addr2line -e build/pc/tmc_pc`). Which of several fade writes lands *last* is what decides whether a scene ends visible or black, and the losing writes leave no trace in `gFadeControl` afterwards — B43 is three writes in one frame where the last is a fade-out |
| `TMC_CUTSCENE_TRACE=1` | which cutscene step machine is running and how it advances — `gUnk_080FD138[gMenu.field_0x3]`, its `overlayType` sub-step, `menuType`, room flag 0, the live `gFadeControl` and the *subtask exit fade* `gUI.fadeType`/`fadeInTime`. Printed on change. A cutscene that stops progressing is a triple that stops moving; B43 is the case where it settles and the exit fade then blacks the screen |
...
| `TMC_HDMA_TRACE=2` | also, per frame, any channel that drove the raster **without having been re-armed** since the last VBlank. On hardware `VBlankIntr`'s `DmaStop(0)` tears channel 0 down every frame and `PerformVBlankDMA` re-arms it only while `gVBlankDMA.ready`, so `active, not re-armed` cannot happen there — it means a dead effect's per-scanline table is still driving a register. B56 is that: `io_off=0x1C` (BG3HOFS) held the light rays' sine wobble for the rest of the room and bent the light shaft when it came back. No picture shows this; the symptom is a *frozen* version of whatever registered the channel |
...
| `TMC_HDMA_KEEPSTALE=1` | restore the pre-B56 behaviour — `DmaStop` stays a no-op, so a channel nobody re-arms keeps replaying its last table. The A/B for a suspected stale channel, and it gives before and after from one binary. It does not reach `DisableVBlankDMA`, which unregistered its channel before the fix too |
