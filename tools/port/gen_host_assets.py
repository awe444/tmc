...
-------------------------
* `assets/gfx_offsets.h` - real `#define offset_X 0x...` for every gfx
                         palette / tile blob in this variant, then
                         `#include "_port_offset_stubs.h"` for catch-all
                         zeros (must live under ``assets/`` so
                         ``#include "assets/gfx_offsets.h"`` resolves with
                         the CMake include layout).
* `assets/map_offsets.h` - real ``#define`` values for every map / tileset
                         entry in ``assets/map.json`` (same offset math as
                         the C++ ``asset_processor``). Does **not** include
                         ``_port_offset_stubs.h`` (``common.c`` includes this
                         header before ``gfx_offsets.h``, which supplies the
                         stub tail once).
...
@dataclass
class MapOffsetEntry:
    """One ``#define offset_<symbol>`` line for ``assets/map_offsets.h``."""

    symbol: str
    value: int  # byte offset from the map block base (same as asset_processor)


def load_map_offset_entries(repo_root: Path, variant: str) -> List[MapOffsetEntry]:
    """Walk ``assets/map.json`` for the active variant.

    Mirrors ``tools/src/asset_processor/main.cpp`` EXTRACT mode for the
    ``calculateOffsets: map_offsets.inc`` region: each asset's macro value
    is ``file_offset - map_base`` where ``map_base`` is that marker's
    ``start`` plus the running per-variant offset from the preceding
    ``offsets`` object.
    """
    cfg_path = repo_root / "assets" / "map.json"
    with cfg_path.open() as f:
        entries = json.load(f)

    variant_off = 0
    map_base: Optional[int] = None
    out: List[MapOffsetEntry] = []
    ...
    for entry in entries:
        ...
        if "offsets" in entry:
            ...
            continue
        if "calculateOffsets" in entry:
            if entry["calculateOffsets"] != "map_offsets.inc":
                continue
            map_base = entry["start"] + variant_off
            continue
        if "path" not in entry:
            continue
        if "variants" in entry and variant not in entry["variants"]:
        ...
        if "name" in entry:
            ...
            symbol = stem_twice(entry["path"])
        out.append(
            MapOffsetEntry(symbol=symbol, value=file_offset - map_base)
        )

    if map_base is None:
        raise SystemExit(
            f"{cfg_path}: no `calculateOffsets: map_offsets.inc` marker"
        )
    return out
...
def write_map_offsets_header(out_path: Path, map_entries: List[MapOffsetEntry]) -> None:
    """Emit ``assets/map_offsets.h`` for the SDL host build.

    Unlike ``gfx_offsets.h``, this file must **not** end with
    ``#include "_port_offset_stubs.h"`` — ``src/common.c`` includes
    ``map_offsets.h`` before ``gfx_offsets.h``, and the latter is the
    single TU that pulls in the catch-all stub after the real gfx
    ``#define`` lines.
    """
    lines = [
        "/* AUTO-GENERATED by tools/port/gen_host_assets.py. DO NOT EDIT. */",
        "/* Real `offset_*` macros for map / tileset assets (see assets/map.json). */",
        "",
        "#ifndef PORT_GENERATED_MAP_OFFSETS_H",
        "#define PORT_GENERATED_MAP_OFFSETS_H",
        "",
    ]
    seen: Set[str] = set()
    for row in map_entries:
        sym = f"offset_{row.symbol}"
        if sym in seen:
            continue
        seen.add(sym)
        lines.append(f"#define {sym} 0x{row.value:x}")
    lines += ["", "#endif /* PORT_GENERATED_MAP_OFFSETS_H */", ""]
    out_path.write_text("\n".join(lines))
...
def cmd_gen(args: argparse.Namespace) -> None:
    ...
    # 1b. assets/map.json -> map offset table (same rules as asset_processor).
    map_entries = load_map_offset_entries(repo_root, args.variant)
    ...
    # 3. write offset headers under assets/ (see CMake: include …/port_assets).
    write_offsets_header(assets_dir / "gfx_offsets.h", assets)
    write_map_offsets_header(assets_dir / "map_offsets.h", map_entries)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    g = sub.add_parser(
        "gen",
        help="generate assets/{gfx,map}_offsets.h + port_rom_assets.c",
    )
