...
{
    uint32_t header = (uint32_t)src[0]
                    | ((uint32_t)src[1] << 8)
                    | ((uint32_t)src[2] << 16)
                    | ((uint32_t)src[3] << 24);
...
void Port_RegisterRamReset(uint32_t flags)
{
    ...
    if (flags & 0x01) memset(gPortEwram, 0, sizeof(gPortEwram));
    if (flags & 0x02) memset(gPortIwram, 0, sizeof(gPortIwram));
    if (flags & 0x04) memset(gPortPltt,  0, sizeof(gPortPltt));
    if (flags & 0x08) memset(gPortVram,  0, sizeof(gPortVram));
    if (flags & 0x10) memset(gPortOam,   0, sizeof(gPortOam));
}
