...
tracked bugs are closed. **B27 is fixed in Hyrule Town and festival town;
its Minish Village half is deferred and is a larger job than the plan assumed
— `docs/town-tileset-residency.md` §8.** **Two things are still open and both
are judgements rather than work: frame time, and B21's light shaft.**
...
| **B27** — town scenery in the outer 40 px drawn from a non-resident tileset | bug tracker | The residual B26 could not reach: matching hardware's choice only guarantees the 240×160 the GBA would have shown. Emulated VRAM now carries a shadow bank above the GBA's 96 KB, both of a slot's tilesets stay resident, and the renderer picks per tile from the tile's own room position. Hyrule Town and festival town; **Minish Village deferred** |
...
| **Minish Village's half of B27** | **Deferred, measured, not started.** Its groups are 32 KB not 8, **three** can be needed on screen at once at 320×240 (72 of ~8,400 camera positions), and each one swaps a **13-bank BG palette group** as well as tile graphics — so the per-tile character path that fixed town cannot fix it alone. Two palette groups are already needed at once at **240×160** on 308 positions, so part of this is the shipping build's own behaviour and that question decides the scope. `docs/town-tileset-residency.md` §8. |
