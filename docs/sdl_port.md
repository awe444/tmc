...
- [x] **PR #8.** (mechanism) Golden-image CI test: snapshot the
  ...
  * `--print-frame-hash` renders one extra frame after the
    ...
    to stdout as `frame-hash: 0x<16 hex>`. Endianness-stable: the
    hash operates on the raw byte stream, so the same logical pixels
    produce the same value on any little- or big-endian host.
